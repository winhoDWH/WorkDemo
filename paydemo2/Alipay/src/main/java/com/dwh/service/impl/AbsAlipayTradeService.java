...
abstract class AbsAlipayTradeService extends AbsAlipayService implements AlipayTradeService {
    ...
    /*// 轮询查询订单支付结果 返回是否成功
    public boolean loopQueryResult(AlipayTradeQueryRequestBuilder builder) {
        AlipayTradeQueryResponse queryResult = null;
        for (int i = 0; i < Configs.getMaxQueryRetry(); i++) {
            Utils.sleep(Configs.getQueryDuration());
            ...
            if (response != null) {
                if (stopQuery(response)) {
                    return querySuccess(queryResult);
                }
                queryResult = response;
            }
        }
    ...
    // 根据外部订单号outTradeNo撤销订单
    public AlipayTradeCancelResponse cancelPayResult(AlipayTradeCancelRequestBuilder builder) {
    ...
    // 异步撤销
    public void asyncCancel(final AlipayTradeCancelRequestBuilder builder) {
